...
//		» The second portion contains only letters of the alphabet.
	...
	public static boolean isEmail (String str) {
		...
		if (singleAtSign(str) && isPrefix(strBeforeAt) && isDomain(strAfterAt)) {
			return true;
		} else {
			return false;
		}
	...
	public static String isUsername (String str) {
		...
		//Evaluating the first condition (containing at least one alphanumeric character):
		int counter_1 = 0;
		...
		/*Evaluating the third condition
		(Containing only alphanumeric characters, periods, dashes, or an exclamation point) */
		int counter_2 = 0;
		for (int i=0; i < str.length(); i++) {
			if (isDomainChar(str.charAt(i)) || str.charAt(i) == '!') {
				counter_2 ++;
			}
		}
		/*Evaluating the fifth condition
		...
		if (str.length() <= 7 //Evaluating the second condition
			&& 
			isSpecialChar(str.charAt(0),false) //Evaluating the fourth condition
			&& 
			counter_1 != 0 //Evaluating the first condition
			&& 
			counter_2 == str.length() //Evaluating the third condition
			&& 
			counter_3 == 0 //Evaluating the fifth condition
			&& 
